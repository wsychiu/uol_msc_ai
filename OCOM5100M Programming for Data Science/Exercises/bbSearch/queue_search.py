...
from bbSearch.tree import * # defines tree data structure


## The main interface function is:
##     search(PROBLEM, INIT_ARG, STRATEGY, MAX_NODES, OPTIONS )
##
...
##     )
##
## INIT_ARG is an argument that is passed to initialise_function when it is called (before the
## actual search is started). Will be ignored if the problem's initialise_func is None.
##
...
def search(problem, init_arg, strategy, max_nodes, options ):
    ...
    if not(initialise_func == None):
       initialise_func(init_arg)  # Call function to initialise problem
