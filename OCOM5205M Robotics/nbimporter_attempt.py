...
def find_notebook(fullname, path=None):
    ...
    for d in path:
        ...
        #print(f'os.path.isfile(nb_path)? {os.path.isfile(nb_path)}')
...
class NotebookLoader(object):
    ...
    def load_module(self, fullname):
        ...
        print(f'NotbookLoader path: {path}')
        ...
        print(f'module fullname: {fullname.split('.', 1)[0]}')
        mod = types.ModuleType(fullname.split('.', 1)[0])
        mod.__file__ = path
        mod.__loader__ = self
        ...
        sys.modules[fullname.split('.', 1)[0]] = mod
...
class NotebookFinder(object):
    ...
    #def find_module(self, fullname="Lesson5_MonteCarloMethods.ipynb", path=None):
    def find_module(self, fullname, path=None):
        nb_path = find_notebook(fullname, path)
        print(f'returned nb_path: {nb_path}')
        if not nb_path:
            return
        
        key = nb_path.rsplit('/', 1)[0] #path
        #print(f'key: {key}')
        '''
        if path:
        ...
        if key not in self.loaders:
            self.loaders[key] = NotebookLoader(key)
        print(self.loaders)
